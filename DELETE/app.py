...
#
# 前提:
# - lib/auth.py, lib/calendar_ics.py が存在
...
def inject_card_css() -> None:
    ...
    st.markdown(
        """
<style>
.upcoming-wrap { display: flex; flex-direction: column; gap: 10px; }
...
.dot {
  width: 10px; height: 10px; border-radius: 999px; margin-top: 6px; flex: 0 0 10px;
...
def render_upcoming_cards(df) -> None:
    ...
    cards_html = ['<div class="upcoming-wrap">']
    ...
    for _, r in upcoming.iterrows():
        ...
        loc_esc = html.escape(loc).replace("\n", "<br/>")

        meta_parts = [f"🕒 {start_str} – {end_str}"]
        if show_location and loc:
            meta_parts.append(f"📍 {loc_esc}")
        ...
        cards_html.append(
            ...
            <div class="event-row">
                ...
                <p class="event-title">{title_esc}</p>
                <div class="event-meta">{meta}</div>
                </div>
                <div class="badge {badge_cls}">{html.escape(status)}</div>
            </div>
            </div>
            """
        )
...
def main() -> None:
    ...
    st.caption("左メニューから「カレンダー」「稽古場一覧」「本番会場」へ移動できます。")

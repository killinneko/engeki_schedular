# app.py
# ホーム：本番カウントダウン＋直近の予定（カード表示）＋Googleマップボタン
#
# 前提:
...
def render_upcoming_cards(df) -> None:
    ...
    for _, r in upcoming.iterrows():
        ...
        with st.container(border=True):
            c_dot, c_main, c_badge, c_btn = st.columns([0.05, 0.67, 0.12, 0.16], vertical_alignment="top")
            ...
            with c_btn:
                if loc:
                    st.link_button("📍 Googleマップ", build_google_maps_search_url(loc), use_container_width=True)
                else:
                    st.write("")
